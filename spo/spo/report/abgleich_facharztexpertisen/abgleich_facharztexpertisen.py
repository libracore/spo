...
def execute(filters=None):
    ...
    for mandat in mandate:
        ...
        pinv_amount = 0
        sinv_amount = 0
        pinv = frappe.db.sql("""SELECT `grand_total`, `posting_date` FROM `tabPurchase Invoice` WHERE `mandat` = '{mandat}' AND `docstatus` != '2' AND `posting_date` >= '{abgleich_ab}' LIMIT 1""".format(mandat=mandat[0], abgleich_ab=abgleich_ab), as_dict=True)
        if len(pinv) > 0:
        ...
        sinv_date = frappe.db.sql("""SELECT `posting_date`, `status` FROM `tabSales Invoice` WHERE `mandat` = '{mandat}' AND `docstatus` != '2' AND `posting_date` >= '{abgleich_ab}' LIMIT 1""".format(mandat=mandat[0], abgleich_ab=abgleich_ab), as_list=True)
        if len(sinv_date) > 0:
            ...
            _data.append(sinv_date[0][1])
            if sinv_date[0][1] == 'Paid':
