...
frappe.ui.form.on('Mandat', {
    refresh: function(frm) {
        // "Erstelle Mandats-Revision" button wenn gebucht
        if (cur_frm.doc.docstatus == 1) {
            frm.add_custom_button(__("Erstelle Mandats-Revision"), function() {
                erstelle_mandats_revision(frm);
            }).addClass("btn-danger")
        }
...
function erstelle_mandats_revision(frm) {
    frappe.confirm(
        'Möchten Sie dieses Mandat Revisionieren?<br>Es wird ein Duplikat angelegt, inkl. allen Anhängen.',
        function(){
            // on yes
            frappe.call({
                method: 'spo.spo.doctype.mandat.mandat.erstelle_mandats_revision',
                args: {
                    'mandat': cur_frm.doc.name
                },
                callback: function(r) {
        ...
        function(){
            // on no
            show_alert('Revisionierung abgebrochen')
        }
    )
}
