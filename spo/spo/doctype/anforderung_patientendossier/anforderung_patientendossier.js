...
frappe.ui.form.on('Anforderung Patientendossier', {
	...
	before_save: function(frm) {
		if (!cur_frm.doc.textbaustein) {
			if (!cur_frm.doc.brieftext && !cur_frm.doc.mahnstufe_1  && !cur_frm.doc.mahnstufe_2) {
			...
			} else if (!cur_frm.doc.brieftext && !cur_frm.doc.mahnstufe_2) {
				...
				cur_frm.set_value('brieftext', brieftext_string);
			} else if (!cur_frm.doc.brieftext && !cur_frm.doc.mahnstufe_1) {
				var brieftext_string = '<p><b>Martin Muster, Musterstrasse 4, 8000 Muster<br>Herausgabe des vollständigen Patientendossiers</b></p><br>' +
			...
			frappe.call({
				...
				"args": {
					"doctype": "SPO Textbausteine",
					"name": cur_frm.doc.textbaustein
				},
				"callback": function(response) {
					var textbaustein = response.message;

					if (textbaustein) {
						cur_frm.set_value('brieftext', textbaustein.textkonserve);
					}
				}
			});
		}
